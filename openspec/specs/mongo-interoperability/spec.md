...
The `rotateDek(namespace)` method SHALL produce vault documents compatible with Java, with per-namespace rotation independence.

#### Scenario: Rotation compatibility
- **WHEN** Node.js rotates a DEK for `default.default.User#phone`
...
- **WHEN** Java encrypts a POJO and store `_t: "DOC"` with BSON binary ciphertext
