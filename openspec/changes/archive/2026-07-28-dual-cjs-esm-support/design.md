...
Node.js 自 v12 起稳定支持 ESM，当前主流库均已提供双格式入口。ESM 消费者通过 `import` 导入纯 CJS 包时，通常只能稳定获得 default export（整个 `module.exports` 对象），命名导入体验不一致，也不利于现代工具链静态分析。

## Goals / Non-Goals

**Goals:**
- ESM 消费者可通过 `import { X } from 'lightcrypto-link-node'` 使用与 `src/index.js` 完全一致的公共 API
...
**理由**: `exports` 字段是 Node.js 官方推荐的条件导出机制。`import` 与 `default` 同时指向 ESM wrapper，可减少未知解析条件误入 CJS 的概率；`require` 继续指向现有 CJS 入口；保留 `main` 兼容旧版 Node/bundler，`module` 供部分打包工具识别。
...
export const {
  // ...必须完整覆盖 src/index.js 的所有导出键
} = cjs;

export default cjs;
```

使用解构赋值从 CJS default import 中提取命名导出，并以自动化测试保证该列表与 `src/index.js` 全量一致。
...
新增两类测试：
- 源码层验证：`test/esm/import-test.mjs` 使用 `import` 语法校验命名导出、default export、引用一致性
- 分发层验证：通过 `npm pack` 产物在临时目录中以包名执行 `import` 与 `require`，验证条件导出生效

考虑到 Jest 对 ESM 支持仍有差异，ESM 验证采用 Node.js 原生执行脚本（可用 `node --test` 或普通断言脚本）。
...
- **[打包工具兼容性]** → 极少数旧版 bundler 可能不识别 `exports` 字段。缓解：保留 `main` + `module` 字段，并在文档中声明该兼容行为为 best-effort 而非强保证。
